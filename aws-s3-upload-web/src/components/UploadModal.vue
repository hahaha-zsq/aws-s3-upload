...
<template>
  <el-modal title="上传" :width="800" v-model:visible="visible" @cancel="visible = false">
    <el-card title="Arco Card" size="small">
      <template #title>
        <el-space>
          <el-upload :show-file-list="false" :auto-upload="false" @change="selectFile" />
      ...
      <el-list size="small">
        <el-list-item v-for="item in state.dataSource" :key="item.uid">
          <el-list-item-meta :title="item.name">
            <template #description>
              ...
              <el-progress v-if="item.progress" :percent="item.progress / 100" />
            </template>
          </el-list-item-meta>
          <template #actions>
